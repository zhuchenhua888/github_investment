...
def main(name_list):
    # 设置参数
    trade_rate = 0.86 / 10000  # 场内基金万分之0.6，买卖手续费相同，无印花税
    ...
    for name in name_list:
        name = name.split('.')[0]
        df.loc[df[f'{name}_mom'] == df[[i.split('.')[0] + '_mom' for i in name_list]].max(axis=1, skipna=True), 'style'] = f'{name}'
    # 相等时维持原来的仓位。
    ...
    for name in name_list:
        name = name.split('.')[0]
        df[f'{name}_net'] = df[f'{name}_close'] / (df[f'{name}_close'].iloc[df[f'{name}_close'].first_valid_index()])
    ...
    # 保存文件
    df.to_csv('大小盘风格轮动.csv', encoding='gbk', index=False)
