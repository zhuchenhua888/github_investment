...
def get_hs_data():
    ...
    try:
        ...
        hs_all_df = ak.stock_zh_index_hist_csindex(
            symbol="000985",
        ...
        # 动态适配日期列和收盘价列
        date_cols = [col for col in hs_all_df.columns if 'date' in col.lower() or '日期' in col]
        close_cols = [col for col in hs_all_df.columns if 'close' in col.lower() or '收盘' in col]
        
        if not date_cols or not close_cols:
            raise ValueError("未找到日期列或收盘价列")
            
        hs_all_df = hs_all_df[[date_cols[0], close_cols[0]]].copy()
        hs_all_df[date_cols[0]] = pd.to_datetime(hs_all_df[date_cols[0]])
        hs_all_df.rename(columns={date_cols[0]: '日期', close_cols[0]: 'hs_all_close'}, inplace=True)
        return hs_all_df
    except Exception as e:
        print(f"获取中证全指价格数据失败: {e}")
        return None
...
def get_hs_all_pe():
    ...
    try:
        ...
        pe_df = ak.stock_zh_index_hist_csindex(symbol="000985", start_date=start_date, end_date=end_date)
        # pe_df = ak.stock_index_pe_lg(symbol="中证全指")
...
def calculate_fed_premium(hs_data, pe_data, bond_data):
    """合并数据并计算FED溢价"""
    try:
        if hs_data is None or pe_data is None or bond_data is None:
            raise ValueError("输入数据不完整")
            
        # 合并数据
        merged_df = hs_data.merge(pe_data, on='日期', how='inner')
...
def plot_fed_premium(data):
    ...
    try:
        ...
        # 左侧纵轴：中证全指指数
        ax1.plot(data['日期'], data['hs_all_close'], color='#1f77b4', linewidth=2, label='中证全指指数')
        ax1.set_xlabel('日期', fontsize=12)
        ax1.set_ylabel('中证全指指数', fontsize=12, color='#1f77b4')
        ax1.tick_params(axis='y', labelcolor='#1f77b4')
        ax1.grid(True, linestyle='--', alpha=0.7)
        
        # 右侧纵轴：FED溢价
        ax2 = ax1.twinx()
        ax2.plot(data['日期'], data['FED溢价'], color='#ff7f0e', linewidth=2, label='FED溢价=1/(中证全指动态市盈率) / 十年期国债收益率')
        ...
        plt.title('估值-中证全指指数与股债比趋势图', fontsize=16)
        plt.tight_layout()
        
        # 保存图表
        chart_file = '估值-中证全指指数与股债比趋势图.png'
...
if __name__ == "__main__":
    # 获取数据
    hs_data = get_hs_data()
    pe_data = get_hs_all_pe()
    bond_data = get_treasury_yield()
    
    # 打印各数据形状用于调试
    if hs_data is not None:
        print(f"中证全指价格数据形状: {hs_data.shape}")
    if pe_data is not None:
        print(f"市盈率数据形状: {pe_data.shape}")
    if bond_data is not None:
        print(f"国债收益率数据形状: {bond_data.shape}")
    
    if hs_data is not None and pe_data is not None and bond_data is not None:
        # 计算FED溢价
        result_df = calculate_fed_premium(hs_data, pe_data, bond_data)
