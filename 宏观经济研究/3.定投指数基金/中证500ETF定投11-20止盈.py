...
# ---------------------- 数据加载 ----------------------
def load_etf_data(file_path='中证500ETF收盘价数据.csv'):
    """从CSV文件加载中证500ETF数据"""
    try:
...
def get_etf_data(max_retries=3):
    """获取中证500ETF历史数据（代码510500）"""
    etf_code = "510500"  # 中证500ETF代码
    retry_count = 0
    while retry_count < max_retries:
        try:
            ...
            df = df.sort_values('date').reset_index(drop=True)
            df.to_csv('中证500ETF收盘价数据.csv', index=False, encoding='utf-8-sig')
            return df
        except Exception as e:
...
def simulate_strategy(data, start_date, 定投金额=1000, 止盈比例=0.15, 最小周期=52):
    ...
    # 计算指标
    总投入 = sum(abs(amount) for date, amount in 现金流 if amount < 0) if 现金流 else 0
    总产出 = sum(amount for date, amount in 现金流 if amount > 0) if 现金流 else 0
    累计收益率 = (总产出 - 总投入) / 总投入 if 总投入 > 0 else 0
    
    # 计算XIRR
    dates = [date for date, amount in 现金流] if 现金流 else []
    amounts = [amount for date, amount in 现金流] if 现金流 else []
    xirr = custom_xirr(amounts, dates) if len(dates) >= 2 else np.nan
    
    return {
        'start_date': start_date, '止盈比例': 止盈比例,
        '总投入': round(总投入, 2), '总产出': round(总产出, 2),
        '累计收益率': round(累计收益率, 4), 'XIRR': round(xirr, 6) if not np.isnan(xirr) else None,
        '止盈次数': 止盈次数, '定投周数': len(定投日)
    }
...
def multi_strategy_comparison(data, 止盈列表=[0.15], 样本数量=30):
    ...
    for start_date in 随机日期:
        ...
        for 止盈比例 in 止盈列表:
            res = simulate_strategy(data=子数据, start_date=start_date, 止盈比例=止盈比例)
            if res:
                结果列表.append(res)
...
# ---------------------- 结果可视化 ----------------------
def plot_results(结果df, prefix="中证500ETF"):
    """绘制收益率对比图表"""
    ...
    plt.title(f'{prefix}不同止盈策略的XIRR分布')
    plt.xlabel('止盈比例')
    plt.ylabel('XIRR真实收益率')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    boxplot_path = f'{prefix}_XIRR箱线图.png'
    plt.savefig(boxplot_path, dpi=300)
    
    # 2. XIRR与累计收益率对比
    plt.figure(figsize=(12, 6))
    sns.scatterplot(data=有效数据, x='累计收益率', y='XIRR', hue='止盈比例', s=80)
    plt.title(f'{prefix}XIRR与累计收益率对比')
    plt.xlabel('累计收益率')
    plt.ylabel('XIRR真实收益率')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    scatter_path = f'{prefix}_XIRR与累计收益率对比.png'
    plt.savefig(scatter_path, dpi=300)
    
    return [boxplot_path, scatter_path]
...
def main():
    ...
    结果df = multi_strategy_comparison(
        data=data, 止盈列表=止盈列表, 样本数量=100  # 100样本
    )
    ...
    # 3. 保存结果
    result_path = '中证500ETF止盈策略对比结果.csv'
    结果df.to_csv(result_path, index=False, encoding='utf-8-sig')
    ...
    stats_path = '中证500ETF止盈策略统计指标.csv'
    统计df.to_csv(stats_path, encoding='utf-8-sig')
    
    # 5. 绘制图表
    图表列表 = plot_results(结果df)
    
    print("中证500ETF止盈策略回测完成!")
    print("XIRR统计结果:")
